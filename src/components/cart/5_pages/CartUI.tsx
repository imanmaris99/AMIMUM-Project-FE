...
const CartUI: React.FC<CartUIProps> = ({ cartResponse, isCartListLoading, selectAll, onToggleAllActivation, onUpdateCart, onRemoveItem }) => {
  const router = useRouter();
  return (
    <div className="mx-auto min-x-[360px] max-w-[400px] relative">
      <TopNavigation>Keranjangku</TopNavigation>

      <CartItemsList isLoading={isCartListLoading} cartList={cartResponse.data} onUpdateCart={onUpdateCart} onRemoveItem={onRemoveItem} />

      <div className="h-[200px] pt-10 bg-color-[#FAFAFA] border-t-4 border-[#E6F1ED] pb-80 px-6">{isCartListLoading ? <CartSummarySkeleton /> : <CartSummary cartResponse={cartResponse} />}</div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white mx-auto max-w-[400px] w-full rounded-t-3xl">
        <div className="flex gap-6 items-center justify-between mt-6 shadow-2xl pt-4 pb-8 px-[30px] flex-grow">
          ...
          {isCartListLoading ? (
            <Button disabled={true} className="bg-gray-400 text-[#E6F1ED] py-2 px-4 rounded-full">
              Checkout (...)
            </Button>
          ) : (
            <Button onClick={() => router.push("/order")} className="bg-[#00764F] text-[#E6F1ED] py-2 px-4 rounded-full">
              Checkout ({cartResponse.data.filter((item) => item.is_active).length})
            </Button>
          )}
        </div>
        <BottomBar />
